    ...
    for (const user of users) {
        try {
            await bot.api.sendMessage(
                user.id,
                "*NEWS*\n\n" +
                "В настоящее время протокол *Hysteria2* невозможно корректно настроить для стабильной работы во всех регионах Российской Федерации при использовании мобильных сетей LTE.\n\n" +

                "*Плановые технические работы сегодня в 23:30 (МСК):*\n\n" +

                "🟩 Протокол *VLESS* будет повторно добавлен и доступен для подключения через LTE на сервере, расположенном в Польше.\n\n" +

                "🟩 Ограничения на объём входящего и исходящего трафика будут полностью сняты. Пропускная способность сервера — до 1000 Мбит/с.\n\n" +

                "‼️ *Важно:* данное обновление является завершающим этапом развития сервиса. В дальнейшем сервис будет функционировать в режиме технической поддержки.",
                { parse_mode: "Markdown" }
            );
        } catch (err) {

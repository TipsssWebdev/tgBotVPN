    ...
    for (const user of users) {
        try {
            await bot.api.sendMessage(
                user.id,
                "☕ *VLESS снова доступен*\n\n" +
                "Это последнее изменение в боте. Переходим фул в режим поддержки ☺️\n\n" +
                "PS: VLESS был добавлен так как на Hysteria практически невозможно адекватно настроить мобильные сети. Особенно так,чтоб они работали во всех регионах РФ.",
                { parse_mode: "Markdown" }
            );
        } catch (err) {
